...
import { chains, importWagerrAddresses, fundAddress, describeExternal } from '../common'
...
function testWallet (chain) {
  ...
  describe('buildBatchTransaction', () => {
    it('should successfully create op_return tx', async () => {
      ...
      const dataScript = wagerr.payments.embed({ data: [data] })
...
describe('Wallet Interaction', function () {
  ...
  describe('Wagerr - JsWallet', () => {
    before(async function () {
      await importWagerrAddresses(chains.wagerrWithJs)
    })

    testWallet(chains.wagerrWithJs)
  })
  ...
  describeExternal('Wagerr - Ledger', () => {
    ...
    testWallet(chains.wagerrWithLedger)
  })
})
