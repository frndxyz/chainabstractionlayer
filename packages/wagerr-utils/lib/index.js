...
import { base58, padHexStart } from '@wagerr-wdk/crypto'
...
function getAddressNetwork (address) {
  // TODO: can this be simplified using just wagerrjs-lib??
  let networkKey
  // bech32
...
function decodeRawTransaction (hex, network) {
  const bjsTx = wagerr.Transaction.fromHex(hex)
  ...
  const vin = bjsTx.ins.map((input) => {
    return {
      ...
      vout: input.index,
      scriptSig: {
        asm: wagerr.script.toASM(input.script),
  ...
  const vout = bjsTx.outs.map((output, n) => {
    ...
    var vout = {
      ...
      n,
      scriptPubKey: {
        asm: wagerr.script.toASM(output.script),
    ...
    try {
      const address = wagerr.address.fromOutputScript(output.script, network)
