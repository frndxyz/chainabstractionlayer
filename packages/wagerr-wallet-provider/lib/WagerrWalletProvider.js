...
export default superclass => class WagerrWalletProvider extends superclass {
  constructor (network, addressType = 'legacy', superArgs = []) {
    if (!AddressTypes.includes(addressType)) {
      throw new Error(`addressType must be one of ${AddressTypes.join(',')}`)
    }

    const baseDerivationPath = `${ADDRESS_TYPE_TO_PREFIX[addressType]}'/7825266'/${network.coinType}'/0'`
  ...
  getPaymentVariantFromPublicKey (publicKey) {
    if (this._addressType === 'legacy') {
      return wagerr.payments.p2pkh({ pubkey: publicKey, network: this._network })
    } else if (this._addressType === 'p2sh-segwit') {
      return wagerr.payments.p2sh({
        redeem: wagerr.payments.p2wpkh({ pubkey: publicKey, network: this._network }),
        network: this._network })
    } else if (this._addressType === 'bech32') {
      return wagerr.payments.p2wpkh({ pubkey: publicKey, network: this._network })
    }
  }
