import KibaProvider from '@wagerr-wdk/kiba-provider'
import { Address } from '@wagerr-wdk/utils'
import * as wagerr from '@wagerr-wdk/wagerrjs-lib'
import { calculateFee, getAddressNetwork } from '@wagerr-wdk/wagerr-utils'
...
export default class WagerrKibaProvider extends KibaProvider { // TODO: for wagerr if required
  ...
  async _buildTransaction (to, value) {
    ...
    const params = {
      ...
      to,
      network: this._network.name === 'wagerr' ? 'Mainnet' : 'Testnet3',
      coin: 'WGR'
    }
  ...
  async signP2SHTransaction (inputTxHex, tx, address, vout, outputScript, lockTime = 0, segwit = false) {
    let sigHash
    if (segwit) {
      sigHash = tx.hashForWitnessV0(0, outputScript, vout.vSat, wagerr.Transaction.SIGHASH_ALL) // AMOUNT NEEDS TO BE PREVOUT AMOUNT
    } else {
  ...
  async getWalletAddress (address) {
    ...
    throw new Error('WagerrJs: Wallet does not contain address')
  }
...
WagerrKibaProvider.version = version
